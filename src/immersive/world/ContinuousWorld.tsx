...
function Palm({ position, lean = 0 }: { position: [number, number, number]; lean?: number }) {
  return <group position={position} rotation={[0, lean, lean * 0.08]}><mesh position={[0, 1.25, 0]}><cylinderGeometry args={[0.13, 0.24, 2.5, 6]} /><meshStandardMaterial color="#785438" roughness={1} /></mesh>{[0, 1.05, 2.1, 3.15, 4.2, 5.25].map((rotation) => <mesh key={rotation} position={[0, 2.48, 0]} rotation={[0.08, rotation, 0.18]} scale={[1.05, 0.16, 0.48]}><sphereGeometry args={[1, 8, 5]} /><meshStandardMaterial color="#3f7650" roughness={0.95} /></mesh>)}</group>;
}
...
export function ContinuousWorld({ stationId, children }: { stationId: StationId; children?: ReactNode }) {
  return <><color attach="background" args={['#c5d9ce']} /><fog attach="fog" args={['#c5d9ce', 28, 68]} /><directionalLight position={[18, 24, 12]} intensity={3.25} color="#ffd08a" /><mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.1, 0]}><planeGeometry args={[54, 46]} /><meshStandardMaterial color="#84995d" roughness={1} /></mesh><mesh rotation={[-Math.PI / 2, 0, 0]} position={[2.5, 0.01, 4]}><planeGeometry args={[6.8, 43]} /><meshStandardMaterial color="#3f9098" roughness={0.42} /></mesh><mesh rotation={[-Math.PI / 2, 0, 0]} position={[2.5, 0.035, 4]}><planeGeometry args={[1.2, 42]} /><meshStandardMaterial color="#8ac3ae" transparent opacity={0.36} roughness={0.25} /></mesh>{[-11, -6, -1, 4, 9, 14].map((z) => <mesh key={z} position={[-8.8, 0.02, z]} rotation={[0, 0.25, 0]}><boxGeometry args={[8.2, 0.26, 2.6]} /><meshStandardMaterial color="#a8ad5a" roughness={1} /></mesh>)}{[-9, -3, 3, 9, 15].map((z) => <mesh key={z} position={[13.6, 0.02, z]} rotation={[0, -0.35, 0]}><boxGeometry args={[6.2, 0.26, 2.7]} /><meshStandardMaterial color="#a8ad5a" roughness={1} /></mesh>)}<InstancedPlants count={105} color="#cfbd62" offset={[-12, -11]} /><InstancedPlants count={75} color="#b4c56b" offset={[9, 5]} />{STATIONS.map((station, index) => <StationMarker key={station.id} position={station.position} index={index} />)}<ActiveStationFeature stationId={stationId} />{children}</>;
}
