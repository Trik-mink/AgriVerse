...
function ActiveStationFeature({ stationId }: { stationId: StationId }) {
  ...
  return <group key={station.id} position={station.position}>
    <mesh position={[0, 0.18, 0]}><cylinderGeometry args={[1.5, 1.8, 0.35, 8]} /><meshStandardMaterial color="#d7b77a" roughness={0.95} /></mesh>
    <mesh position={[0, 1.35, 0]}><coneGeometry args={[1.55, 1.7, 4]} /><meshStandardMaterial color="#b75d45" roughness={0.9} /></mesh>
  </group>;
}
...
export function ContinuousWorld({ stationId, children }: { stationId: StationId; children?: ReactNode }) {
  return (
    <>
      ...
      <directionalLight position={[18, 24, 12]} intensity={2.4} />
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.1, 0]}>
        <planeGeometry args={[54, 46]} />
        <meshStandardMaterial color="#799e62" roughness={1} />
      </mesh>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[2.5, 0.01, 4]}>
      ...
      {[-11, -6, -1, 4, 9, 14].map((z) => <mesh key={z} position={[-8.8, 0.02, z]} rotation={[0, 0.25, 0]}><boxGeometry args={[8.2, 0.26, 2.6]} /><meshStandardMaterial color="#89a854" roughness={0.95} /></mesh>)}
      {[-9, -3, 3, 9, 15].map((z) => <mesh key={z} position={[13.6, 0.02, z]} rotation={[0, -0.35, 0]}><boxGeometry args={[6.2, 0.26, 2.7]} /><meshStandardMaterial color="#89a854" roughness={0.95} /></mesh>)}
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[-4.8, 0.04, 5]}>
      ...
      <ActiveStationFeature stationId={stationId} />
      {children}
    </>
  );
}
